...
class PassFilterResolver:
        ...
        if triage_rows:
            for kind in ("risky", "structural", "symbolic", "clean", "covered", "uncovered"):
                if not resolved[kind]:
                    resolved[kind] = PassFilterResolver._pass_names_from_triage_rows(triage_rows, kind=kind)

        summary_pass_evidence = list(PassFilterResolver._summary_first(summary, "pass_evidence", []))
        fallback_checks = {
    ...
    def _is_uncovered_pass(evidence: dict[str, Any] | None, symbolic: dict[str, Any] | None) -> bool:
        return PassFilterResolver._is_clean_pass(evidence, symbolic) and not PassFilterResolver._is_covered_pass(
            evidence, symbolic
        )
...
class ReportFilters:
    ...
    def resolve_mismatch_view(
        ...
        if mismatch_map:
            ...
            persisted_rows = list(ReportFilters._summary_first(summary, "observable_mismatch_by_pass", []))
            counts_by_pass = {
